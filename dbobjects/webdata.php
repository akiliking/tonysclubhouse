...
//    $dbQuery = "SELECT * from djdb_tblNews Order by date, timestamp desc LIMIT $index , $count";
  ...
  function addClubhouseEvent($CategoryId, $title, $active, $summary, $detail, $location, $url, $active, $start_date, $end_date, $userfile,$urllink) {
	if (!$active){$active = 1;}
	if (!$CategoryId){$CategoryId = 'NULL';}
	$Title = $this->sqlsafe($Title);
	$Type = $this->sqlsafe($Type);
	...
	echo is_uploaded_file($userfile);
  	...
  	$dbQuery = "INSERT INTO djdb_tblEvent (title,description, detail, location, event_url, start_date, end_date, active, published) 
				VALUES ('$title', '$detail', '$detail', '$location','$url', '$start_date', '$end_date', 1, 1)";
	$errmsg = "Error: Failed to Insert Record! --  ";
	$success = "$title was successfully Added!";
	$dbResultSet = mysql_query($dbQuery)  or die ($errmsg .mysql_error() ." for: ".$dbQuery);
