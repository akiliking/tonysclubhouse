...
{
    show_report(array('which' => 'jquery', 'category' => 'entry_form'));
}
...
if (url_parse($_url, $_url_parts))
{
    ...
    if (!empty($_hosts))
    {
        foreach ($_hosts as $host)
        {
            ...
            {
                show_report(array('which' => 'jquery', 'category' => 'error', 'group' => 'url', 'type' => 'external', 'error' => 1));
            }
        }
    }
}
...
{
    show_report(array('which' => 'jquery', 'category' => 'error', 'group' => 'url', 'type' => 'external', 'error' => 2));
}
...
if (!$_config['allow_hotlinking'] && isset($_SERVER['HTTP_REFERER']))
{
    ...
    if ($is_hotlinking)
    {
        ...
        {
            case 1:
                show_report(array('which' => 'jquery', 'category' => 'error', 'group' => 'resource', 'type' => 'hotlinking'));
                break;
            case 2:
...
do
{
    ...
    if ($_socket === false)
    {
        show_report(array('which' => 'jquery', 'category' => 'error', 'group' => 'url', 'type' => 'internal', 'error' => $err_no));
    }
    ...
    if ($_response_code == 401 && isset($_response_headers['www-authenticate']) && preg_match('#basic\s+(?:realm="(.*?)")?#i', $_response_headers['www-authenticate'][0], $matches))
    {
        ...
        {
            show_report(array('which' => 'jquery', 'category' => 'auth', 'realm' => $matches[1]));
        }
    }
}
while ($_retry);
...
if (!isset($_proxify[$_content_type]))
{
    ...
    if ($_content_length !== false)
    {
        ...
        {
            show_report(array('which' => 'jquery', 'category' => 'error', 'group' => 'resource', 'type' => 'file_size'));
        }
